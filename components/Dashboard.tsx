...
import { User, WeightEntry, TabType } from '../types';
import { Plus, Scale, ChevronRight, TrendingUp, Dumbbell, Utensils, CircleDot, Award } from 'lucide-react';
...
const Dashboard: React.FC<Props> = ({ user, weights, onTabChange, setWeights, isWorkoutActive }) => {
  const [showWeightModal, setShowWeightModal] = useState(false);
  const [newWeight, setNewWeight] = useState('');
  const [loading, setLoading] = useState(false);

  // currentWeight: last weight entry or initial_weight
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 flex flex-col justify-between hidden lg:flex">
          <div className="flex justify-between items-start mb-4">
            <h3 className="text-slate-400 font-black uppercase text-[10px] tracking-widest">Objetivo</h3>
            <TrendingUp size={20} className="text-emerald-500" />
          </div>
          <div className="text-2xl font-black text-slate-800">{user.goal || 'Não definido'}</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <SectionCard
          ...
          desc={user.role === 'profissional' ? 'Gerenciar treinos dos alunos' : 'Ver sua rotina de hoje'}
          icon={<Dumbbell className="text-blue-500" size={32} />}
          onClick={() => onTabChange('training')}
        />
        <SectionCard
          title="Plano Alimentar"
          desc={user.role === 'profissional' ? 'Ajustar dietas dos alunos' : 'Consultar dieta do dia'}
